...
get_ipython().run_cell_magic('time', '', '#importar galeries\nimport openpyxl\nimport numpy as np\nimport matplotlib.pyplot as plt\nfrom metpy.plots import SkewT\nfrom metpy.units import units\nimport seaborn as sns\n\n#Valors a canviar al canviar d\'any:\nestació="Albuquerque"\nany=2022\ndies_febrer=28\nexcel=openpyxl.load_workbook("2022_tfg_Albuquerque.xlsx")#obrim l\'excel d\'Alburquerque\n#excel=openpyxl.load_workbook("2022_tfg_Grand_Junction.xlsx")#obrim l\'excel de Grand Junction\n#excel=openpyxl.load_workbook("2022_tfg_Dodge_City.xlsx")#obrim l\'excel de Dodge City\n\n#per fer servir aquest programa en una estació que no sigui ALburquerque, es posa un "#" davant la línia\n#que obra l\'excel d\'Alburquerque i se\'l treu de la línia que obre l\'excel de l\'estació que escollim\n')
...
# A partir d'aquí el programa és per comparar estacions i comparar els valors pressos durant el dia i la nit.

# In[11]:


#Aquí és guarden els valors necessàris per que després, al utilitzar el codi "comparar estacions annex",
#poguem obtenir les gràfiques per comparar els valors entre estacions.

#perqué això sigui possible s'ha d'utilitzar primer tot aquest codi, "perfils de Temperatura annex" en els valors
#pressos a les 3 estacions en el mateix any. Per utilitzar el codi en una estació que no sigui la d'ALburquerque, 
#es treu el símbol "#" de la línia del segon bloc de programació d'aquest programa que fagi obrir l'excel que necessitem 
#i s'ha de possar el símbol "#" davant de la línia que obra l'excel d'Alburquerque

#guardem en un file les temperatures de les 00 de la nit dels radiosondatges
with open(f"{estació}{any}_00nit.csv", 'w') as file:
    for row in matriu[0]:
        file.write(','.join(map(str, row)) + '\n')
#guardem en un file les temperatures a les 12 del dia dels radiosondatges
with open(f"{estació}{any}_12dia.csv", 'w') as file:
    for row in matriu[1]:
        file.write(','.join(map(str, row)) + '\n')
#guardem en un file les mitjanes del skewT de temperatura de les 00 de la nit dels radiosondatges
with open(f"{estació}{any}_skewT_00.csv", 'w') as file:
    file.write(','.join(map(str, matriu_skewT[:,1])))

#guardem en un file les mitjanes del skew T de temperatura de rosada a les 00 de la nit dels radiosondatges
with open(f"{estació}{any}_skewTd_00.csv", 'w') as file:
    file.write(','.join(map(str, matriu_skewTr[:,1])))



# A partir d'aquí es fan les gràfiques per comparar els valors pressos de dia (a les 17h) i i aquells presso de nit (a les 5h).

# In[13]:


#comparar
restanit_dia=matriu00-matriu12#dades de la temepratura de la nit menys la temperatura del dia

#gràfiques de la resta nit menys dia
for i in np.arange(0,len(pressionsTot)):
    plt.plot(dates_00,restanit_dia[i,:], label=f'{pressionsTot[i]}hPa',color=custom_colors[i])

    #decoració
    plt.title(f"{any} diferència de temperatura entre el dia i la nit")
    plt.xticks(dies_mesos,rotation=90)
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.ylabel("Temperatura (ºC)")
...
#la resta de les mitjanes 00 nit - 12 dia
print(matriu_skewT[:,1])
print(matriu_skewT12[:,1])
mitj_00_12=matriu_skewT[:,1]-matriu_skewT12[:,1]
pressions2=["830.0","700.0","610.0","500.0","400.0","300.0","200.0","150.0","100.0","50.0","30.0","20.0"]
print(mitj_00_12)

plt.plot(np.array(pressions2, dtype=float),mitj_00_12, color="b")
plt.title(f"Mitjana a cada pressió de 00 de la nit - 12 del migdia {estació}")
